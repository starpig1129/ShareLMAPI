...
ShareLMAPI 是一個本地語言模型共享 API，使用 FastAPI 提供接口，允許不同的程序或不同的設備共享同一個本地模型，從而減少資源消耗。並支援流式生成和多種模型配置方法。
...
## ShareLMAPI 與 oLLama 的差異比較

| **功能**                   | **ShareLMAPI**                               | **oLLama**                                      |
|----------------------------|----------------------------------------------|-------------------------------------------------|
| **模型專注**                | 支援多種模型 (如 `transformers` 與 `PEFT`)    | 主要專注於 LLaMA 模型及其變體                   |
...
| **自訂化**                 | 高度可自訂，允許動態調整 `max_length`、`temperature` 以及其他生成參數 | API 自訂化功能較少，針對 LLaMA 調整              |
